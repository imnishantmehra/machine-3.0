...
export function ContentGenerationQueue() {
  ...
  const [queueItems, setQueueItems] = useState<ContentQueueItem[]>([
    { id: "1", type: "entity-person", name: "John Smith", source: "Entity Recognition", selected: false },
    { id: "2", type: "entity-organization", name: "Acme Corporation", source: "Entity Recognition", selected: false },
    { id: "3", type: "entity-location", name: "San Francisco", source: "Entity Recognition", selected: false },
    { id: "4", type: "topic", name: "Digital Marketing Strategy", source: "Topic Modeling", selected: false },
    { id: "5", type: "word-cloud", name: "Content", source: "Word Clouds", selected: false },
    { id: "6", type: "sentiment", name: "Positive Sentiment", source: "Micro Sentiments", selected: false },
    { id: "7", type: "hashtag", name: "#DigitalMarketing", source: "Hashtag Generator", selected: false },
    { id: "8", type: "keyword", name: "content strategy", source: "Keywords", selected: false },
    {
      id: "9",
    ...
    { id: "10", type: "topical-map", name: "Content Creation Process", source: "Topical Map", selected: false },
    {
      id: "11",
      type: "hashtag",
  ...
  const [activeTab, setActiveTab] = useState<"queue" | "generated" | "scheduled">("queue")
  ...
  const queueCount = queueItems.filter((item) => !item.generated && !item.scheduled).length
  const generatedCount = queueItems.filter((item) => item.generated && !item.scheduled).length
  const scheduledCount = queueItems.filter((item) => item.scheduled).length

  // Get selected items for icon display
  const selectedItems = queueItems.filter((item) => item.selected && !item.generated && !item.scheduled)
  ...
  const handleSelectAll = (selected: boolean) => {
    setQueueItems((items) =>
      items.map((item) => {
        ...
        } else if (activeTab === "generated" && item.generated && !item.scheduled) {
          return { ...item, selected }
  ...
  const handleSelectItem = (id: string, selected: boolean) => {
    setQueueItems((items) => items.map((item) => (item.id === id ? { ...item, selected } : item)))
  }
  ...
  const handleRemoveSelected = () => {
    setQueueItems((items) => {
      if (activeTab === "queue") {
        return items.filter((item) => !(item.selected && !item.generated && !item.scheduled))
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedItems.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {selectedItems.map((item) => (
            <div key={item.id} className="flex items-center bg-white rounded-full px-3 py-1 shadow-sm border">
      ...
      <Card>
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "queue" | "generated" | "scheduled")}>
            ...
            <TabsContent value="queue" className="space-y-4">
              {queueItems.filter((item) => !item.generated && !item.scheduled).length > 0 ? (
                <>
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="select-all-queue" onCheckedChange={(checked) => handleSelectAll(!!checked)} />
                    ...
                    <div className="flex space-x-2">
                      <Button
                        ...
                        disabled={!queueItems.some((item) => item.selected && !item.generated && !item.scheduled)}
                      >
                      ...
                      <Button
                        ...
                        disabled={
                          isGenerating ||
                          !queueItems.some((item) => item.selected && !item.generated && !item.scheduled)
                        }
                      >
                        {isGenerating ? (
                          <>
                  ...
                  {generationSuccess && (
                    <div className="bg-green-50 border border-green-200 rounded-md p-3 mb-4 flex items-center">
                      ...
                      <p className="text-green-700">
                        Content successfully generated! View in the Generated Content tab.
                      </p>
                    </div>
                  )}
                        ...
                        <div key={item.id} className="flex items-start p-3 border rounded-md">
                          ...
                          <div className="ml-3 flex-1">
                            <div className="flex items-center">
                              {getItemIcon(item.type)}
                              <Label htmlFor={`item-${item.id}`} className="ml-2 font-medium cursor-pointer">
                                {item.name}
                              </Label>
                            </div>
                            <Badge className={`mt-1 ${getSourceBadgeColor(item.source)}`}>{item.source}</Badge>
                          </div>
                        </div>
                      ))}
                  </div>
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold mb-2">Content Queue Empty</h3>
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    Add items to the queue from Entity Recognition, Topic Modeling, or Research Assistant tabs.
                  </p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="generated" className="space-y-4">
              {queueItems.filter((item) => item.generated && !item.scheduled).length > 0 ? (
                <>
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="select-all-generated" onCheckedChange={(checked) => handleSelectAll(!!checked)} />
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                          ...
                          <div>
                            ...
                            >
                              {DAYS.map((day) => (
                                <ToggleGroupItem key={day} value={day} aria-label={day} className="px-2 py-1 text-xs">
                                  {day}
                                </ToggleGroupItem>
                              ))}
                            </ToggleGroup>
                        ...
                        {(!selectedPlatform || !selectedTime || selectedDays.length === 0) && (
                          <div className="flex items-center text-amber-600 mb-4 text-sm">
                            <AlertCircle className="h-4 w-4 mr-2" />
                            Please select a platform, time, and at least one day to schedule content.
                          </div>
                        )}
                        ...
                        <Button
                          ...
                          disabled={isGenerating || !selectedPlatform || !selectedTime || selectedDays.length === 0}
                        >
                          {isGenerating ? (
                            <>
                              <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                        ...
                        {schedulingSuccess && (
                          <div className="bg-green-50 border border-green-200 rounded-md p-3 mt-4 flex items-center">
                            <Check className="h-5 w-5 text-green-500 mr-2" />
                            <p className="text-green-700">
                              Content successfully scheduled! View in the Production Schedule tab.
                            </p>
                          </div>
                        )}
                      </CardContent>
                        ...
                        <Card key={item.id} className="overflow-hidden">
                          <div className="flex items-start p-4">
                            ...
                            <div className="ml-3 flex-1">
                              <div className="flex items-center">
                                {getItemIcon(item.type)}
                                <Label htmlFor={`generated-${item.id}`} className="ml-2 font-medium cursor-pointer">
                                  {item.name}
                                </Label>
                              </div>
                              <Badge className={`mt-1 ${getSourceBadgeColor(item.source)}`}>{item.source}</Badge>
                          ...
                          <div className="px-4 pb-4 pt-2">
                            <Textarea
                              ...
                              onChange={(e) => {
                                setQueueItems((items) =>
                                  items.map((i) => (i.id === item.id ? { ...i, generatedContent: e.target.value } : i)),
                                )
                              }}
                ...
                <div className="text-center py-12">
                  <FileText className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold mb-2">No Generated Content</h3>
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    Select items from the queue tab and click "Generate Content" to create content.
                  </p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="scheduled" className="space-y-4">
              {queueItems.filter((item) => item.scheduled).length > 0 ? (
                <>
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="select-all-scheduled" onCheckedChange={(checked) => handleSelectAll(!!checked)} />
                    ...
                    <div className="flex space-x-2">
                      <Button
                        ...
                        size="sm"
                        onClick={handleRemoveSelected}
                        disabled={!queueItems.some((item) => item.selected && item.scheduled)}
                      >
                        <Trash2 className="h-4 w-4 mr-2" />
                        Remove Selected
                      </Button>
                    ...
                    {queueItems
                      ...
                      .map((item) => (
                        <Card key={item.id} className="overflow-hidden">
                          <div className="flex items-start p-4">
                            ...
                            <div className="ml-3 flex-1">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  {getItemIcon(item.type)}
                                  <Label htmlFor={`scheduled-${item.id}`} className="ml-2 font-medium cursor-pointer">
                                    {item.name}
                                  </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                  {item.scheduledPlatform && getPlatformIcon(item.scheduledPlatform)}
                          ...
                          <div className="px-4 pb-4 pt-2">
                            <Textarea value={item.generatedContent} readOnly className="min-h-[100px] mt-2" />
                            <div className="flex justify-between mt-2">
                              <div className="flex items-center">
                                <Clock className="h-4 w-4 mr-1 text-blue-500" />
                                <span className="text-sm text-gray-600">
                                  Scheduled for {item.scheduledPlatform} on {item.scheduledDate} at {item.scheduledTime}
                                </span>
                              </div>
                              <Button size="sm" variant="outline">
                                <RefreshCw className="h-4 w-4 mr-2" />
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    Select items from the Generated Content tab and use "Send to Production" to schedule content.
                  </p>
                </div>
              )}
            </TabsContent>
